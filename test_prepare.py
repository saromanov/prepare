...
class TestPrepare(object):
    ...
    def test_addColumn(self):
        data = self.item
        result = data.addColumn('append1', [1,2,3,4,5,6]).toMatrix()
    ...
    def test_mergeColumn(self):
        result = prepare.Prepare().read('./testdata/test1.csv').mergeColumn(path='./testdata/test2.csv')
        view = result.toMatrix()
        assert len(view[0]) == 7
